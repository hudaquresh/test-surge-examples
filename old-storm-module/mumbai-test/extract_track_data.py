...
def create_storm_file(storm, output_path="mumbai.storm"):
    ...
    with open("../../gulf/ike/ike-h80/ike.storm", 'r') as ref_storm_file:
        RRP = []
        max_winds = []
        central_pressure = []
        radius_max_winds = []
...
# "        DDDDDDDDDD      XXXX  XXX  DDDDS  DDDDS DDD" % 
        ...
        for n in xrange(len(storm['track'][0])):
            ...
            storm_file.write(" %s%s %s%s" 
                                    % (str(int(storm['track'][1][n] *
10)).rjust(4), lat_dir,
                                       str(int(storm['track'][0][n] *
10)).rjust(4), lon_dir))
            storm_file.write(" %s" % str(int(storm['max_winds'][n])).rjust(3))
            storm_file.write(" %s" % str(int(storm['radius_max_winds'][n]*0.539957)).rjust(4))
...
def extract_data(path, mask_dist=numpy.infty, mask_category=0):
    ...
    for n in xrange(lon.shape[0]):
        ...
        if (numpy.any(storm['dist_mumbai'] < mask_dist) and
            ...
            storms.append(storm)
            # print(storm['radius_max_winds'])  
            print(storm['max_winds'])  
...
if __name__ == '__main__':
    ...
    path = "/rigel/home/hq2152/data/Mumbai3_io_ncep_reanalcal.mat"
    if len(sys.argv) > 1:
        path = sys.argv[1]

    storms = extract_data(path, mask_dist=0.2, mask_category=1)
    print(len(storms))
